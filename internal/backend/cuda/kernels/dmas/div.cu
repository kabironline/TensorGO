...
// -------------------------------------------------------------------------
__global__ void div_vec4_kernel(
...
int cuda_div(float *d_a, float *d_b, float *out, int size, cudaStream_t stream) {
    ...
    if (vec_count > 0) {
        ...
        div_vec4_kernel<<<gridSize, blockSize, 0, stream>>>(d_a, d_b, out, vec_count);
    }

    // 4. Launch Scalar Kernel (only if size is not a divtiple of 4)
    if (remainder > 0) {
        // Offset is where the vectorized part stopped
        int offset = vec_count * 4;
        div_scalar_kernel<<<1, 32, 0, stream>>>(d_a, d_b, out, size, offset);
    }
