...
// -------------------------------------------------------------------------
// Kernel 1: Vectorized Add (The Workhorse)
// Processes 4 floats (128 bits) per thread to maximize memory bandwidth.
// -------------------------------------------------------------------------
__global__ void add_vec4_kernel(
...
// -------------------------------------------------------------------------
// Kernel 2: Scalar Add (The Cleanup)
...
int cuda_add(float *d_a, float *d_b, float *out, int size, cudaStream_t stream, cublasHandle_t handle) {
    if (!d_a || !d_b || !out || size <= 0) {
        printf("cuda_add: Invalid parameters\n");
        return -1;
    }
    ...
    if (vec_count > 0) {
        ...
        add_vec4_kernel<<<gridSize, blockSize, 0, stream>>>(d_a, d_b, out, vec_count);
    }

    // Launch Scalar Kernel (only if size is not a multiple of 4)
    if (remainder > 0) {
        int offset = vec_count * 4;
        add_scalar_kernel<<<1, 32, 0, stream>>>(d_a, d_b, out, size, offset);
    }

    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        printf("cuda_add: Kernel launch failed with error %d: %s\n", err, cudaGetErrorString(err));
        fflush(stdout);
        return 1;
    }
    ...
    if (err != cudaSuccess) {
        printf("cuda_add: Kernel execution failed with error %d: %s\n", err, cudaGetErrorString(err));
        fflush(stdout);
        return 1;
    }
    return 0;
