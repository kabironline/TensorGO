...
func (t *Tensor) Im2Col(kH, kW, stride, padding int) *Tensor {
	if len(t.Shape) != 4 {
		panic("Im2Col requires a 4D tensor [N, C, H, W]")
	}

	// Delegate to device-specific implementation
	resData := t.Device.Im2Col(t.Data, t.Shape, t.Strides, kH, kW, stride, padding)

	N, C, H, W := t.Shape[0], t.Shape[1], t.Shape[2], t.Shape[3]
	outH := (H+2*padding-kH)/stride + 1
	outW := (W+2*padding-kW)/stride + 1

	out := NewTensor(resData, []int{C * kH * kW, N * outH * outW}, t)

	out.Backward = func() {
		gradT := t.Device.Col2Im(out.Grad, t.Shape, t.Strides, kH, kW, stride, padding)
